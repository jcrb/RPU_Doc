...
```{}
$\pm \Sexpr{x}$ (ne pas oublier les parenthèses)
```

if else
-------
Ne jamais mettre else en début de ligne (après newline) car provoque une erreur:
```
...
```
ref: https://stat.ethz.ch/R-manual/R-devel/library/base/html/Control.html
...
```
ref: https://google.github.io/styleguide/Rguide.xml#curlybraces
...
Pour supprimer des lignes vides dans une matrice:
```
# sample data
...
m[!is.finite(m)] <- 0
```
Pour supprimer des colonnes vides:
```
m[, !colSums(!is.finite(m))]
```
...
- Pour supprimer le commentaire (date, n° de version) rajouter xtable(t, comment = FALSE)

- Pour afficher un tableau en html print.xtable(xtable(t, comment = FALSE), type = "html")

- Pour afficher un tableau en latex print.xtable(xtable(t, comment = FALSE, caption = "un titre"), type = "latex")
...
Pour supprimer le commentaire en début de tableau, rajouter header=FALSE (http://stackoverflow.com/questions/16465255/how-to-suppress-stargazer-information-in-output-file)
...
Boxplot: trier (reorder)
------------------------
Pour rendre le graphique plus parlant, on réorganise les boxplots pour qu'ils soient affichés par moyenne d'age croissante. D'abord on réorganise les FINESS avec la fonction *reorder* qui demande trois paramétres: le variable factor é reorganiser (finess), la variable qui servira de supoort é la réorganisation (age) et la fonction de tri (mean). S'agissant de la moyenne ne pas oublier na.rm=TRUE pour éviter un affichage aberrant. Puis on affiche les boxplots par ordre de moyenne d'age croissant. 
```{}
a<-with(d1,reorder(FINESS,AGE,mean,na.rm=TRUE))
boxplot(AGE~a,data=d1,xlab="Etablissement",ylab="Age (années)",col="orange")
```

Boxplot: réorganiser par ordre croissant
----------------------------------------
on veut affichher sous forme de boxplot le temps de passage dans les différents ES d'Alsace. On dispose d'un dataframe destemps moyens de passage par jour (ligne) et par finess (colonne). Avec apply on détermine la valeur médiane (ou moyenne) pour chaque colonne et on les trie. Puis on réorganise le dataframe initial avant de le boxplotter:
```{}
# par ordre croissant de médiane
...
ou http://stackoverflow.com/questions/2169118/installing-all-cran-packages-that-are-not-already-installed
...
- colnames(dataframe)[n°colonne] <- "name"
- ex: colnames(dx)[1] <- "date"
...
Personaliser l'axe des x
========================

- empêcher de dessinner l'axe des x [source](activite_tension.R)
```
plot(..., xaxt = "n")
```
...
Transformer Latin 1 (windows) en UTF8 (linux)
---------------------------------------------
La plupart des fichiers disponibles via Open Data ont été préparés avec windows et sont de fait en codage Latin1.
```{}
...
df1$id %in% idNums1
```
produit un vecteur logique. Pour le négativer il faut négativer la totalité du vecteur:
```{}
!(df1$id %in% idNums1)
```
[source](http://stackoverflow.com/questions/9852832/r-not-in-subset)

Boxplot
=======

Augmenter/diminuer la taille des labels d'un boxplot: utiliser préalablement par(cex.lab = 1.5) ou par(cex.axis = 1.5)

Graphisme
=========

desiner hors limites
--------------------
```{}
 par(xpd=TRUE)
```
ref: http://stackoverflow.com/questions/3932038/plot-a-legend-outside-of-the-plotting-area-in-base-graphics
...
Error in readRDS(file) : unknown input format voir http://stackoverflow.com/questions/6473831/readrdsfile-in-r
